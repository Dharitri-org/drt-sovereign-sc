...
use transaction::{GasLimit, StolenFromFrameworkDctTokenData, Transaction, TransferData};
    ...
    + dharitri_sc_modules::pause::PauseModule
{
    ...
    /// Create an Dharitri -> Sovereign transaction.
    #[payable("*")]
    #[endpoint]
    fn deposit(
        &self,
        ...
        for payment in &payments {
            ...
            if payment.token_nonce > 0 {
                let current_token_data = self.blockchain().get_dct_token_data(
                    &own_sc_address,
                ...
                all_token_data.push(StolenFromFrameworkDctTokenData::default());
            }
        ...
        let final_payments: FinalPayment<Self::Api> = self
            ...
            .with_dct_transfer(fees_payment)
            .execute_on_dest_context();

        self.send()
            .direct_non_zero_dct_payment(&caller, &final_payments.remaining_tokens);
