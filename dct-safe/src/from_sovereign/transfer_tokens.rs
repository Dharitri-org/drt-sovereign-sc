...
use transaction::{
    BatchId, GasLimit, PaymentsVec, StolenFromFrameworkDctTokenData, Transaction, TxNonce,
};
    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
        ...
        for sov_tx in &signed_transactions {
            ...
            for (token, token_data) in sov_tx.tokens.iter().zip(sov_tx.token_data.iter()) {
                let token_roles = self
                    .blockchain()
                    .get_dct_local_roles(&token.token_identifier);
    ...
    fn mint_tokens(
        ...
        all_token_data: ManagedVec<StolenFromFrameworkDctTokenData<Self::Api>>,
    ) -> PaymentsVec<Self::Api> {
        let own_sc_address = self.blockchain().get_sc_address();
        let mut output_payments = PaymentsVec::new();
        for (payment, token_data) in payments.iter().zip(all_token_data.iter()) {
            let token_balance = self.blockchain().get_dct_balance(
                &own_sc_address,
            ...
            let mx_token_id_state = self
                .sovereign_to_dharitri_token_id(&payment.token_identifier)
                .get();
            ...
            if payment.token_nonce == 0 {
                self.send()
                    .dct_local_mint(&mx_token_id, 0, &payment.amount);

                output_payments.push(DctTokenPayment::new(mx_token_id, 0, payment.amount));

                continue;
            }

            let token_nonce = self.send().dct_nft_create(
                &mx_token_id,
                &payment.amount,
                &token_data.name,
