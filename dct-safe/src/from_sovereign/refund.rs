...
pub struct CheckMustRefundArgs<'a, M: ManagedTypeApi> {
    pub token: &'a DctTokenPayment<M>,
    pub roles: DctLocalRoleFlags,
    ...
    + max_bridged_amount_module::MaxBridgedAmountModule
{
    fn check_must_refund(&self, args: CheckMustRefundArgs<Self::Api>) -> bool {
        ...
        if token_balance < args.token.amount {
            if args.token.token_nonce == 0 {
                if !args.roles.has_role(&DctLocalRole::Mint) {
    ...
    fn has_nft_roles(&self, payment: &DctTokenPayment, roles: DctLocalRoleFlags) -> bool {
        if !roles.has_role(&DctLocalRole::NftCreate) {
            return false;
        }

        if payment.amount > NFT_AMOUNT && !roles.has_role(&DctLocalRole::NftAddQuantity) {
            return false;
        }
