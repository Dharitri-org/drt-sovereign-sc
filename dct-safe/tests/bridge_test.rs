...
use transaction::{
    transaction_status::TransactionStatus, StolenFromFrameworkDctTokenData, Transaction,
};
...
#[test]
fn init_test() {
    let _ = BridgeSetup::new(dct_safe::contract_obj, fee_market::contract_obj);
}

#[test]
fn transfer_two_tokens_to_sov_ok() {
    let mut bridge_setup = BridgeSetup::new(dct_safe::contract_obj, fee_market::contract_obj);
    ...
    bridge_setup.b_mock.check_dct_balance(
        &bridge_setup.user,
        FEE_TOKEN_ID,
    ...
    bridge_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                tokens.push(DctTokenPayment::new(
                ...
                token_data.push(StolenFromFrameworkDctTokenData::default());
                token_data.push(StolenFromFrameworkDctTokenData::default());
...
fn transfer_token_from_sov_no_roles_refund() {
    ...
    bridge_setup
        ...
        .execute_tx(
            ...
            &rust_biguint!(0),
            |sc| {
                // transactions were converted into Dharitri -> Sov for refunding
