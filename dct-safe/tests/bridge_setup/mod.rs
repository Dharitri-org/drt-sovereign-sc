...
where
    BridgeBuilder: 'static + Copy + Fn() -> dct_safe::ContractObj<DebugApi>,
    FeeMarketBuilder: 'static + Copy + Fn() -> fee_market::ContractObj<DebugApi>,
{
    ...
    pub bridge_wrapper: ContractObjWrapper<dct_safe::ContractObj<DebugApi>, BridgeBuilder>,
    ...
    FeeMarketBuilder: 'static + Copy + Fn() -> fee_market::ContractObj<DebugApi>,
{
    pub fn new(bridge_builder: BridgeBuilder, fee_market_builder: FeeMarketBuilder) -> Self {
        ...
        b_mock.set_dct_balance(&user, FUNGIBLE_TOKEN_ID, &rust_biguint!(TOKEN_BALANCE));
        b_mock.set_dct_balance(&user, FEE_TOKEN_ID, &rust_biguint!(TOKEN_BALANCE));
