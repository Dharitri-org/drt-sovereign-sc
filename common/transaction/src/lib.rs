...
pub type TxAsMultiValue<M> = MultiValue7<
    ...
    ManagedVec<M, StolenFromFrameworkDctTokenData<M>>,
...
// Temporary until Clone is implemented for DctTokenData
#[derive(
    TopDecode, TopEncode, NestedDecode, NestedEncode, TypeAbi, Debug, ManagedVecItem, Clone,
)]
pub struct StolenFromFrameworkDctTokenData<M: ManagedTypeApi> {
...
impl<M: ManagedTypeApi> Default for StolenFromFrameworkDctTokenData<M> {
    fn default() -> Self {
        StolenFromFrameworkDctTokenData {
...
impl<M: ManagedTypeApi> From<DctTokenData<M>> for StolenFromFrameworkDctTokenData<M> {
    fn from(value: DctTokenData<M>) -> Self {
        StolenFromFrameworkDctTokenData {
...
pub struct Transaction<M: ManagedTypeApi> {
    ...
    pub tokens: ManagedVec<M, DctTokenPayment<M>>,
    pub token_data: ManagedVec<M, StolenFromFrameworkDctTokenData<M>>,
