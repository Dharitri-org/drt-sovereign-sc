...
pub struct FinalPayment<M: ManagedTypeApi> {
    pub fee: DctTokenPayment<M>,
    pub remaining_tokens: DctTokenPayment<M>,
}
    ...
    + bls_signature::BlsSignatureModule
{
    ...
    fn subtract_fee(
        ...
        if !self.is_fee_enabled() || self.users_whitelist().contains(&original_caller) {
            self.send()
                .direct_dct(&caller, &payment.token_identifier, 0, &payment.amount);

            return FinalPayment {
                fee: DctTokenPayment::new(payment.token_identifier.clone(), 0, BigUint::zero()),
        ...
        self.send()
            .direct_non_zero_dct_payment(&caller, &final_payment.remaining_tokens);
    ...
    fn subtract_fee_same_token(
        ...
        FinalPayment {
            fee: DctTokenPayment::new(payment.token_identifier.clone(), 0, total_fee),
            remaining_tokens: payment,
        }
    }
    ...
    fn subtract_fee_any_token(
        ...
        args.payment = DctTokenPayment::new(
        ...
        // User pays 100 MOAX, which by asking the pair you found out is 2000 RIDE
        // Then the cost for the user is (1500 RIDE * 100 MOAX / 2000 RIDE = 75 MOAX)
        ...
        FinalPayment {
            ...
            remaining_tokens: DctTokenPayment::new(
